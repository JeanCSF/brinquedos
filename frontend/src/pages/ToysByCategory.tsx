...
const ToysByCategory: React.FC = () => {
    const { categoryName } = useParams<{ categoryName: string }>();
    const paths = [
        { name: "Home", path: "/" },
        { name: `Categoria - ${categoryName}`, path: `/category/${categoryName}` },
    ];
    ...
    const fetchToys = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/toys/api/category/${categoryName}`);
